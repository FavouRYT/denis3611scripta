    ...
    public class Commands : IScript
    {
       ...
       */
        ...
        */
        ...
        [Command("weather")]
        public void setWeatherCMD(IPlayer player, string weather)
        {
            if (player.AdminLevel() <= 8) { HUDHandler.SendNotification(player, 4, 5000, "Keine Rechte."); return; }
            player.Emit("Client:Weather:SetWeather", weather);
            HUDHandler.SendNotification(player, 1, 2500, $"Wetter wurde umgestellt  auf {weather}");
        }

        [Command("eon")]
        public void EngineOnCmd(IPlayer player)
        {
            try
            {
                if (player.AdminLevel() <= 1) { HUDHandler.SendNotification(player, 4, 5000, "Keine Rechte."); return; }
                ...
                HUDHandler.SendNotification(player, 1, 5000, $"Der Motor des Autos wurde Erfolgreich gestartet.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void SetATMCmd(IPlayer player, string name)
        {
            try
            {
                ...
                HUDHandler.SendNotification(player, 2, 5000, $"ATM mit dem Namen > {name} erfolgreich gesetzt.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
